...
export default function PortfolioSection() {
  return (
    <section id="portfolio" className="py-[clamp(100px,12vw,160px)] bg-black">
          ...
          {portfolioItems.map((item, i) => (
            <motion.div
              ...
              className={`group relative overflow-hidden rounded-sm border border-[rgba(200,212,224,0.04)] cursor-pointer transition-all duration-300 hover:border-[rgba(74,158,255,0.2)] hover:-translate-y-0.5 ${
                item.size === "large" ? "md:col-span-2 lg:col-span-2 lg:row-span-2 min-h-[280px] lg:min-h-[460px]" : item.size === "medium" ? "min-h-[240px]" : "min-h-[200px]"
              }`}
            >
              <div className={`absolute inset-0 transition-transform duration-700 group-hover:scale-[1.03] ${
                item.bg === 1 ? "bg-gradient-to-br from-[#0a1628] via-[#050d1a] to-[#020810]" : item.bg === 2 ? "bg-gradient-to-br from-[#080f20] via-[#040c1a] to-[#020810]" : "bg-gradient-to-br from-[#060e1e] via-[#040a16] to-[#020810]"
              }`}>
                <div className="absolute inset-0 flex items-center justify-center opacity-60">
                  <div className={`rounded-full border border-[rgba(74,158,255,0.08)] animate-[spin_25s_linear_infinite] ${item.size === "large" ? "w-[200px] h-[200px]" : "w-[100px] h-[100px]"}`} />
                  <div className={`absolute rounded-full border border-[rgba(74,158,255,0.12)] animate-[spin_15s_linear_infinite_reverse] ${item.size === "large" ? "w-[130px] h-[130px]" : "w-[60px] h-[60px]"}`} />
                </div>
              </div>
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex flex-col justify-end p-8">
                ...
                <div className="font-[var(--font-michroma)] text-[9px] tracking-[0.25em] text-[var(--blue-glow)] uppercase mb-3 opacity-80">{item.type}</div>
                <div className={`font-[var(--font-michroma)] text-chrome tracking-[0.03em] mb-2 ${item.size === "large" ? "text-[22px]" : "text-[15px]"}`}>{item.title}</div>
                <div className="text-[13px] text-[rgba(200,212,224,0.4)] tracking-[0.05em]">{item.sub}</div>
              </div>
            </motion.div>
          ))}
        </div>
